...
class GraphWin(tk.Canvas):
    ...
    def onDrag(self, event):
        ...
        self.mouseX = event.x
        self.mouseY = event.y
        self.setCoords(self.leftUpper[0], self.leftUpper[1], self.leftUpper[0] + self.scale[0], self.leftUpper[1] + self.scale[1])        
    
    def onMouseWheel(self, event):
        if event.delta > 0:
            self.scale[0] /= event.delta / 360.0 + 1
            self.scale[1] /= event.delta / 360.0 + 1
        else:
            self.scale[0] *= -event.delta / 360.0 + 1
            self.scale[1] *= -event.delta / 360.0 + 1
        self.setCoords(self.leftUpper[0], self.leftUpper[1], self.leftUpper[0] + self.scale[0], self.leftUpper[1] + self.scale[1])        
        print(self.leftUpper, self.scale)
